...
function AdminAnalytics() {
  ...
  useEffect(() => {
    const fetchAllAnalytics = async () => {
      try {
        ...
        const [
          usersRes, salonsRes, retentionRes,
          genderRes, ageRes,
          pointsRes, vouchersRes, loyalRes,
          topSalonsRes, topServicesRes, topProductsRes
        ] = await Promise.all([
        ...
        if (!usersRes.ok || !salonsRes.ok || !retentionRes.ok || 
            !genderRes.ok || !ageRes.ok || !pointsRes.ok || 
            !vouchersRes.ok || !loyalRes.ok || !topSalonsRes.ok || 
            !topServicesRes.ok || !topProductsRes.ok) {
  ...
  const maxGenderCount = Math.max(...genderDistribution.map(g => g.total_count), 1);
  const maxAgeCount = Math.max(...ageDemographics.map(a => a.total_count), 1);
      ...
      <section className="analytics-section">
        ...
        <div className="stats-cards">
          <div className="stat-card">
            <h3>Total Users</h3>
            <p className="stat-value">{totalUsers?.toLocaleString() || 0}</p>
          </div>
          <div className="stat-card">
            <h3>Total Salons</h3>
            <p className="stat-value">{totalSalons?.toLocaleString() || 0}</p>
          </div>
          <div className="stat-card">
            <h3>Active Users (30 days)</h3>
            <p className="stat-value">{activeUsers?.toLocaleString() || 0}</p>
            {totalUsers > 0 && (
              <p className="stat-subtitle">
                {((activeUsers / totalUsers) * 100).toFixed(1)}% retention
              </p>
            )}
          </div>
        </div>
          ...
          <div className="bar-chart">
            {genderDistribution.length > 0 ? (
              genderDistribution.map((item, index) => {
                const percentage = ((item.total_count / maxGenderCount) * 100);
                return (
                  <div key={index} className="bar-item">
                    ...
                    <div className="bar-value">
                      {totalUsers > 0 ? ((item.total_count / totalUsers) * 100).toFixed(1) : 0}%
                    </div>
                  </div>
                );
              })
          ...
          <div className="bar-chart">
            {ageDemographics.length > 0 ? (
              ageDemographics.map((item, index) => {
                const percentage = ((item.total_count / maxAgeCount) * 100);
                return (
                  <div key={index} className="bar-item">
      ...
      <section className="analytics-section">
        ...
        <div className="stats-cards">
          <div className="stat-card">
            ...
            <p className="stat-value">{pointsRedeemed?.toLocaleString() || 0}</p>
          </div>
          <div className="stat-card">
            <h3>Total Vouchers Redeemed</h3>
            <p className="stat-value">{vouchersRedeemed?.toLocaleString() || 0}</p>
          </div>
        </div>
        ...
        <div className="modal-overlay" onClick={() => setShowSalonsModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowSalonsModal(false)}>×</button>
            </div>
        ...
        <div className="modal-overlay" onClick={() => setShowServicesModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowServicesModal(false)}>×</button>
            </div>
            <div className="modal-body">
              {modalLoading ? (
                <p>Loading...</p>
