...
function SalonRegistration() {
  ...
  const handlePart2Submit = async (servicesData, employeesData) => {
    setIsLoading(true);
    setMessage("");

    // Use the data passed from Part2 (already converted to names)
    const servicesToSubmit = servicesData || newSalon.services || [];
    const employeesToSubmit = employeesData || newSalon.employees || [];

    try {
      // Validate that services exist
      if (!servicesToSubmit || servicesToSubmit.length === 0) {
      ...
      console.log("=== Sending Part 2 data to backend ===");
      console.log("Services:", JSON.stringify(servicesToSubmit, null, 2));
      console.log("Employees:", JSON.stringify(employeesToSubmit, null, 2));
      ...
      if (response.ok) {
        console.log("✅ Services/Employees added successfully:", data);
        alert("Salon registered successfully! Please log in to continue.");
        navigate('/login');
      } else {
